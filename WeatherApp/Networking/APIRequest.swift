...
class APIRequest {
    ...
    func fetchWeatherData(for coordinates: CLLocationCoordinate2D, completion: @escaping (Result<CurrentWeather, Error>) -> Void) {
        ...
        guard let urlComponents = URLComponents(string: "api.openweathermap.org/data/2.5/weather?lat=\(coordinates.latitude)&lon=\(coordinates.longitude)&appid=\(API_KEY)&units=metric") else { return }
